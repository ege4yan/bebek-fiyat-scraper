...
def amazon_tara(max_sayfa=1):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False, slow_mo=70) # Xvfb için False yapıldı
            ...
            for card in cards:
                try:
                    ...
                    if title == "İsim Bulunamadı" and link_el:
                        ...
                        if len(alt_text) > 15: title = alt_text
                    if title == "İsim Bulunamadı" or not full_link: continue
                    ...
                    if whole_el:
                        ...
                        if offscreen: fiyat = offscreen.text.strip().replace('₺', 'TL')
...
def trendyol_tara(max_sayfa=1):
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False, slow_mo=50) # Xvfb için False yapıldı
        context = browser.new_context(
            viewport={'width': 1920, 'height': 1080},
            user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
...
def n11_tara(max_sayfa=1):
    ...
    with sync_playwright() as p:
        ...
        for sayfa_no in range(1, max_sayfa + 1):
            ...
            if "Aradığın sayfa bulunamadı" in page.content(): break
                ...
                if '/urun/' in href or ('bebek' in href and '-' in href and not 'arama' in href and not 'kategori' in href):
                    ...
                    if any('TL' in t for t in text_blocks):
                        ...
                        if toplam_fiyat == "Fiyat Bulunamadı" and fiyatlar:
                            ...
                            if olasi_toplam: toplam_fiyat = re.sub(r'\s+', ' ', olasi_toplam[-1]).strip()
...
def hepsiburada_tara(max_sayfa=1):
    ...
    with sync_playwright() as p:
        ...
        context = browser.new_context(
            viewport={'width': 1920, 'height': 1080},
            user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36",
            java_script_enabled=True, bypass_csp=True, locale="tr-TR", timezone_id="Europe/Istanbul"
        )
        page = context.new_page()
        stealth_sync(page)
        ...
        for sayfa_no in range(1, max_sayfa + 1):
            ...
            if not cards: cards = soup.find_all("li", attrs={"data-index": True})
            eklenen_urun = 0
...
if __name__ == "__main__":
    print("🚀 Tam Teşekküllü Tarama Motoru Playwright ve Xvfb ile Başlatılıyor...\n")
    toplam_urunler = []
